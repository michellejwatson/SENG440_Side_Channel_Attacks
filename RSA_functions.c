...
void RSA_encryption_decryption(unsigned long long int plaintext, unsigned long long int E, unsigned long long int N, unsigned long long int D, unsigned long long int m){
    ...
    //srand(time(NULL));
    //int delay_milliseconds = rand() % 41 + 10; // Generate a random number between 10 and 50
    //usleep(delay_milliseconds * 1000); // Convert microseconds to milliseconds and sleep
